...
app.use(express.static('dist'))
 ...
 app.delete('/api/persons/:id',(request, response, next)=>{
  ...
  .then((result)=>response.status(204).end()).catch(error=>next(error))
 })
...
app.use((request, response, next)=>{ 
      ...
      morgan( request.method === 'GET'? 'tiny' : 
...
app.get("/info", async (request, response) => {
  ...
  const time = new Date()
  response.send(`<p><h3>Phonebook has information for ${count} people </h3></p>
    <p><h3>${time}</h3></p>`
  ).catch(error=> {
    console.log(error)
...
app.post("/api/persons", (request, response, next) => {
 ...
 Person.findOne({name})
  ...
  if(existingname){
     ...
     .then((updatedperson)=>{
      response.status(200).json({message: "name already exist, replace the phone number?", updatedperson})
    })
     .catch(error=>next(error))
  } else{
